 ...
 * @return
 */
int computeMaxSumBruteForce(vector<int> &a){
    int maxSum = 0;
    for (int i = 0; i < a.size(); i++)
    {
        int currentSum = a[i];
        maxSum = max(currentSum, maxSum);
        int k = (i + 1 == a.size()) ? 0 : i + 1;
        for (int j = k; j != i; j++)
        {
            if (i == 0 && j == a.size())
            {
 ...
 * @return
 */
int computeMaxCircularSum(vector<int> &a){
    int totalSum = 0;
...
int main(){
    vector<int> a = {-7,32, -11, 21, 18, 35, -26, -17, 35, -12, -38, -33, 32, 16, 44, 11, -40, -21, 2, 27, -35, 21, -37, -12, 1};
    int maxSumUsingBruteForce = computeMaxSumBruteForce(a);
    cout << "Max Sum of circular array using brute force : " << maxSumUsingBruteForce << endl;

    int maxSumUsingOptimisedApproach = computeMaxCircularSum(a);
    cout << "Max Sum of circular array using optimised approach: " << maxSumUsingOptimisedApproach << endl;
